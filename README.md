...
Hãy mở dự án trong Unity và thực hiện theo 5 bước đơn giản sau:
...
2. Chọn `_Managers` và kéo thả lần lượt 5 script quản lý cốt lõi từ thư mục `Assets/Scripts/` vào cửa sổ Inspector của nó:
   - `GameManager.cs` (Quản lý thời gian, ngày/giờ và chuyển giai đoạn game)
   - `PlayerStats.cs` (Quản lý các chỉ số sinh tồn: Máu, Thể lực, Tinh thần, Stress)
   - `WeatherManager.cs` (Mô phỏng thời tiết nắng nóng, gió Lào, bão lũ, mực nước lụt)
   - `StorageManager.cs` (Quản lý kho chứa đồ, cơ chế thối mốc nông sản tươi, chế biến đồ khô)
   - `CommunityManager.cs` (Quản lý điểm Nghĩa Tình/Karma và cơ chế đổi công hỗ trợ chằng nhà chống bão)
...
1. Tạo một Empty GameObject tên là `SoilCell` và gắn script `SoilCell.cs` vào nó.
2. Tạo một Empty GameObject tên là `AncestralAltar` và gắn script `AncestralAltar.cs` vào nó.
3. Tạo hai Empty GameObject đại diện cho láng giềng:
   - GameObject thứ nhất đặt tên là `NPC_BacNam`, gắn script `NPCCharacter.cs`, và chỉnh trường **Character Type** thành `Bac Nam`.
   - GameObject thứ hai đặt tên là `NPC_OTham`, gắn script `NPCCharacter.cs`, và chỉnh trường **Character Type** thành `O Tham`.
...
2. Kéo thả lần lượt 3 script sau vào `_UI_Tester`:
   - `FrameworkMainMenuUI.cs` (Menu bắt đầu điện ảnh)
   - `FrameworkDebugUI.cs` (Bảng kiểm soát sinh tồn)
   - `FrameworkTester.cs` (Bộ chạy kịch bản thử nghiệm gieo trồng ảo)
