...
*   **`GameManager`**: Quản lý vòng đời trò chơi, quản lý thời gian ngày/đêm và điều phối các hệ thống con.
*   **`PhaseManager`**: Quản lý trình tự 4 giai đoạn thiên tai và thời gian chuyển dịch giữa các ngày cốt truyện.
...
*   **`SoilCell`**: Đại diện cho 1 ô đất trồng, lưu trữ ẩm độ đất, mức độ dinh dưỡng, độ sỏi đá và tham chiếu đến `CropInstance` đang gieo.
...
*   **`NPCCharacter`**: Chứa thông tin NPC (O Thắm, Bác Năm) và mở ra danh sách lựa chọn tương tác đơn giản (Vần công, quyên góp, nhận hạt).
...
### G. Endings (Kết cục)
*   **`EndingManager`**: Bật canvas kết thúc game, chọn kịch bản kết cục tương ứng dựa trên tổng điểm Nghĩa Tình tích lũy.
...
| **`PhaseManager`** | Chuyển đổi trạng thái Phase thiên tai tự động. Bật Banner UI Loa phát thanh xã. | Cập nhật tham số thời tiết mới sang `WeatherManager`. |
...
| **`SoilCell`** | Xử lý các tương tác nông nghiệp cục bộ: dọn đá cát, tiếp nhận hạt gieo, cập nhật độ ẩm đất và kiểm tra ngập úng. | Gửi yêu cầu trừ hạt giống sang `InventoryManager`. |
...
| **`EndingManager`** | Đọc chỉ số Nghĩa Tình cuối ngày thứ 4 để quyết định kết cục game (Đất sỏi cằn, Lá lành đùm lá rách, hay Đất cày nở hoa). | Tắt di chuyển nhân vật trên `PlayerController`. |
