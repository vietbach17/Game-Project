...
Các hệ thống từng bị thiếu trong giai đoạn chuyển hướng hiện đã được triển khai ở mức playable demo:
* **Simple Community Events (Đã triển khai):** Các sự kiện cộng đồng hiện được kích hoạt theo 2 phase chính: **Phase 1 - Before the Storm** (chuẩn bị bão, tích lũy Vần công, gia cố nhà cửa) và **Phase 2 - After the Storm** (bão lũ, cứu trợ, phù sa, tái thiết).
* **Ending System (Đã triển khai):** Canvas Panel hiện màn hình đánh giá cốt truyện ở cuối **Phase 2 - After the Storm** dựa trên điểm Nghĩa Tình đạt được (`nghiaTinh >= 80` là Best, `>= 40` là Normal, `< 40` là Sad Ending).
* **Runtime phase enum note:** Code hiện vẫn có các mốc enum chi tiết `LapNghiep`, `GioLao`, `ChuanBiBao`, `MuaBao`, `PhuSa`. Khi đọc/viết docs, gom `LapNghiep` + `GioLao` + `ChuanBiBao` vào Phase 1 và `MuaBao` + `PhuSa` vào Phase 2.
...
1. Kết nối `TutorialManager.InitializeTutorial()` để tutorial tự khởi động khi bắt đầu game mới.
2. Tạo/gán 3 ảnh minh họa Ending cho Sad / Normal / Best.
3. Bổ sung thoại địa phương miền Trung cho O Thắm, Bác Năm, Cụ Bảy, Bé Tí theo 2 phase chính.
4. Hoàn thiện gameplay cho `FloodBarrier` / bao cát / tấm chắn lũ nếu còn thời gian.
5. Triển khai hiệu ứng gameplay nhẹ cho `item_non_la` (giảm stress thời tiết khi làm việc ngoài trời).
...
Bản mẫu hiện tại đạt khoảng **92% tiến độ sẵn sàng cho Demo**. Core gameplay, 2 phase chính, farming loop, Nghĩa Tình/Vần công, NPC proximity UI, tutorial, weather/flood, phù sa, roof survival, inventory/shop, altar/bếp ga/thuyền thúng và Ending system đều đã hoạt động ở mức demo. Khối lượng còn lại chủ yếu là polish, auto-start tutorial, thêm hình minh họa ending, thoại địa phương và một vài cơ chế phụ như FloodBarrier/Nón Lá.
...
*   **Floating Quest Markers**:
    ...
    *   **Trigger Rules**:
        *   **O Thắm**: Shows `!` during **Phase 1 - Before the Storm** for preparation support (`eventOThamFoodCompleted` false; runtime mốc `GioLao`/chuẩn bị) and **Phase 2 - After the Storm** for recovery (`eventVillageRecoveryCompleted` false; runtime mốc `PhuSa`).
        *   **Bác Năm**: Shows `!` during **Phase 2 - After the Storm** for storm/flood support (`eventBacNamStormCompleted` false; runtime mốc `MuaBao`) and recovery (`eventVillageRecoveryCompleted` false; runtime mốc `PhuSa`).
...
*   **Context-Sensitive Dialogue FALLBACKS & Proximity Options**:
    ...
    *   Wrote fallback dialog responses reflecting local elder wisdom for Cụ Bảy and comforting comments for Bé Tí across the current runtime milestones, grouped under the 2 main design phases.
    ...
    *   Implemented the **Cứu trợ lương thực** option for Cụ Bảy (active during **Phase 2 - After the Storm**, runtime mốc `MuaBao`) which checks and consumes 5 total food items from `StorageManager` (Fresh Crops, Preserved Crops, or Noodles) and rewards +20 Nghĩa Tình.
*   **Automated Smoke Test Verification**:
    *   Executed an automated Editor C# validation script verifying Bep Gas bounds, Altar setup, NPC animators, colliders, and Phase 2 storm/flood option stacks. **All tests successfully PASS**.
