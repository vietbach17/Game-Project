...
* **NPC Bác Năm House Setup**: Replaced old placeholder house model with the newly generated custom low-poly stylized models for Bác Năm's house and his bamboo daybed (chõng tre). Created a custom editor script `SetupBacNamHouse.cs` (`Sown In Stone -> Setup Bac Nam House` menu item) to automate the setup process: creating materials from textures, instantiating the models under `BacNam_House`, auto-scaling (house 4.5m, daybed 1.2m, Bác Năm 1.7m), aligning pivots flat to the ground (Y=0), rotating the container 180 degrees to face south, positioning the daybed next to Bác Năm in the yard, and configuring appropriate physical colliders and interaction triggers.
* **Player's House (Thành's House) Setup**: Replaced old placeholder house model with the newly generated custom low-poly stylized model for Thành's house (`Meshy_AI_Stylized_low_poly_3D__0620084846_texture.fbx`). Created a custom editor script `SetupThanhHouse.cs` (`Sown In Stone -> Setup Thanh House` menu item) to automate the setup process: creating materials, instantiating the model under `Thanh_House`, auto-scaling to a height of 4.5m, aligning the pivot to the ground (Y=0), rotating the container 180 degrees to face the player/road, and configuring an accurate physical BoxCollider.
* **Ancestral Altar Setup**: Replaced old placeholder altar object with the custom low-poly stone altar model (`Meshy_AI_tôi_muốn_làm_mộ_0613091059_texture.fbx`). Created a custom editor script `SetupAltar.cs` (`Sown In Stone -> Setup Altar` menu item) to automate setup: creating materials, instantiating under `AncestralAltar`, auto-scaling to 1.8m height, aligning pivot flat to the ground (Y=0), and configuring a BoxCollider trigger.
